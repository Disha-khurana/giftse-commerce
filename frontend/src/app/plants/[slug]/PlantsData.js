...
import {
  ...
  plantData,
  price,
} from "@/app/components/common/Plants";

function PlantsData({ slug }) {
        ...
        <div className="bg-white py-4 w-full lg:w-[30%]">
          <div className="sticky top-36">
            ...
            {materialData[slug] && (
              <div className="px-4 py-2 border-b border-b-gray-300">
                <div className="flex justify-between py-2 ">
                  <h4 className="font-semibold">Pot material</h4>
                  {show.material == "material" ? (
                    <FiMinus onClick={() => handleChange("material")} />
                  ) : (
                    <HiOutlinePlusSm onClick={() => handleChange("material")} />
                  )}
                </div>
                {show.material && (
                  <div>
                    {materialData[slug]?.map((item, index) => (
                      <div key={index} className="flex items-center gap-3 mb-2">
                        <input type="checkbox" />
                        <span className="text-slate-500 flex items-center tracking-wide ">
            ...
            {plantData[slug] && (
              <div className="px-4 py-2 border-b border-b-gray-300">
                <div className="flex justify-between py-2 ">
                  <h4 className="font-semibold">Plant Type</h4>
                  {show.type == "type" ? (
                ...
                {show.type && (
                  <div>
                    {plantData[slug]?.map((item, index) => (
                      <div key={index} className="flex items-center gap-3 mb-2">
                        <input type="checkbox" />
                        <span className="text-slate-500 flex items-center tracking-wide ">
                          <span className="capitalize">{item.plant}</span> (
                          {item.count})
                        </span>
                      </div>
                    ))}
            ...
            {plantCategoriesData[slug] && (
              <div className="px-4 py-2 ">
                ...
                {show.category && (
                  <div>
                    {plantCategoriesData[slug]?.map((item, index) => (
                      <div key={index} className="flex items-center gap-3 mb-2">
                        <input type="checkbox" />
                        <span className="text-slate-500 flex items-center tracking-wide ">
