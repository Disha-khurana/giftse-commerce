...
function Calender() {
    ...
    return (
        <div className="px-5 md:px-12">
            <h3 className="text-xl lg:text-[25px] mb-4 font-semibold tracking-normal">
                Celebrations Calendar
            </h3>
            <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
                {festive.map((item, index) => (
                    <div key={index} className="flex flex-col items-center overflow-hidden rounded-[26px] shadow-md">
                        <Image
                            width={400}
                            height={400}
                            src={item.img}
                            alt={item.name}
                            className="rounded-t-[26px] w-[90%] md:w-full transition-transform duration-300 ease-in-out transform hover:scale-105"
                        />
